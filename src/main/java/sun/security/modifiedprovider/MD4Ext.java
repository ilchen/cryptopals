...
public final class MD4Ext extends DigestBase1 {
    ...
    /**
     * Hack for https://cryptopals.com/sets/4/challenges/30
     */
    @Override
    ...
    public static void spreadIntsToBytesLittle(int[] inInts, int inOff, byte[] outBytes, int outOff, int intLen) {
        for (int i = 0; i < intLen; ++i) {
            ...
            outBytes[outOff + i * 4 + 2] = (byte) (inInts[inOff + i] >>> 16);
            outBytes[outOff + i * 4 + 3] = (byte) (inInts[inOff + i] >>> 24);
        }
    }
    ...
    void implDigest(byte[] out, int ofs) {
        ...
        ByteBuffer  bb = ByteBuffer.allocate(8).order(ByteOrder.LITTLE_ENDIAN).putLong(bitsProcessed);
        System.arraycopy(bb.array(), 0, buffer, 56, 8);
        implCompress(buffer, 0);

        spreadIntsToBytesLittle(state, 0, out, ofs, 4);
    }
    ...
    void implCompress(byte[] buf, int ofs) {
        squashBytesToIntsLittle(buf, ofs, x, 0, 16);
