...
The order of the whole group is 8 ⋅ (2<sup>252</sup> + 27742317777372353535851937790883648493). The whole group isalso cyclic and
...
i.e. m<sub>enc</sub> = g<sub>1</sub><sup>&gamma;</sup>. Substituting it into (2) yields:
  c<sup>n</sup> = g<sub>1</sub><sup>&gamma;·n</sup> (2)

By Lagrange theorem we know that m<sub>enc</sub><sup>n</sup> is bound to be in a group with max 8 elements. This makes it trivial to learn 3 lowest order
bits of &gamma;, which indirectly gives you 3 bits of knowledge about m<sub>enc</sub>. Implement logic to figure out the 3 lowesr order bits of messages encrypted with Multiplicative ElGamal where Elligator 2 is used to encode plaintext messages to curve points.
...
* [The code for the attack](https://github.com/ilchen/cryptopals/blob/master/src/main/java/com/cryptopals/Set9.java#L77-L126)
